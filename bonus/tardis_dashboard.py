...
if menu == "Delay prediction":
    ...
    if st.button("Predict delay"):
        ...
        if not isinstance(liste, list):
            ...
            st.success(
                f"Predicted data for trip from {departure_station} to {arrival_station} in {month}: {float(liste[1] * 100):.2f}% risk of the train being cancelled, {float(liste[0]):.2f} minutes of delay"
            )
